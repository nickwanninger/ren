...
namespace ren {
  ...
  class RenderGraph {
   public:
    ...
    inline TaskBuilder pass(const char *name) {
      return TaskBuilder(*this, std::string(name));
    }
    ...
    GraphHandle createImage(const std::string_view &name, const GraphImageSpec &spec, GraphAccess initialAccess);
    // Get the Image resource for a given handle.
    ren::ImageRef getImage(GraphHandle handle) const { return get<ren::ImageResource>(handle)->image; }
    ...
    // Execute the render graph, scheduling all tasks unconditionally.
    void run(class Renderer &renderer);
    ...
    void dumpSSA(void);
    // Print the render graph as DOT format for Graphviz visualization
    void printDot() const;
    ...
    // Schedules ALL tasks unconditionally, with ordering based on read/write dependencies.
    // NOTE: this is slow right now. Will look into caching and invalidating.
    RenderSchedule compile();
    ...
    ref<T> get(GraphHandle handle) const {
      ...
      if (!casted) {
        throw std::runtime_error(fmt::format("Resource {} has type {} but requested type does not match", handle, it->second->type));
      }
   ...
   private:
    ...
    // Helper: detect cycles in the dependency graph
    void detectAndReportCycle();

    // Helper: report a cycle with the given path
    void reportCycle(const std::vector<RenderTask*>& path);

    // Helper: perform topological sort to produce a valid execution order
    void topologicalSort(RenderTask *task, std::vector<RenderTask *> &outOrder, std::unordered_set<RenderTask *> &visited);
  ...
  inline T &RenderGraph::addTask(const char *name, Args &&...args) {
    static_assert(std::is_base_of<RenderTask, T>::value, "when adding a task to the render graph, it must be derived from ren::RenderTask");
  ...
  inline RenderTask &RenderGraph::addTask(const char *name, RenderTaskLambda &&func) { return addTask<LambdaRenderTask>(name, std::move(func)); }
