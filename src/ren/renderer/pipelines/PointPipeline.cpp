...
namespace ren {
  ...
  PointPipeline::PointPipeline(VkDescriptorSetLayout descriptorSetLayout) {
    ...
    this->vertexShader =
        std::make_shared<ren::Shader>("shaders/point.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
    this->fragmentShader =
        std::make_shared<ren::Shader>("shaders/point.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
