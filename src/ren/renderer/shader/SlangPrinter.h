...
#define SCOPED_OBJECT() ScopedObject scopedObject##__COUNTER__(this)
    ...
    void printOffset(slang::VariableLayoutReflection* variableLayout,
                     slang::ParameterCategory layoutUnit) {
      printOffset(layoutUnit, variableLayout->getOffset(layoutUnit),
    ...
    void printCumulativeOffsets(slang::VariableLayoutReflection* variableLayout,
                                AccessPath accessPath) {
      key("cumulative");
    ...
    CumulativeOffset calculateCumulativeOffset(slang::VariableLayoutReflection* variableLayout,
                                               slang::ParameterCategory layoutUnit,
                                               AccessPath accessPath) {
    ...
    void printCumulativeOffset(slang::VariableLayoutReflection* variableLayout,
                               slang::ParameterCategory layoutUnit, AccessPath accessPath) {
    ...
    CumulativeOffset calculateCumulativeOffset(slang::ParameterCategory layoutUnit,
                                               AccessPath accessPath) {
      ...
      switch (layoutUnit) {
        ...
        case slang::ParameterCategory::DescriptorTableSlot:
          ...
          for (auto node = accessPath.deepestParameterBlock; node != nullptr; node = node->outer) {
            result.space +=
                node->variableLayout->getOffset(slang::ParameterCategory::SubElementRegisterSpace);
          }
          break;
      }
      return result;
    ...
    void printOffsets(slang::VariableLayoutReflection* variableLayout, AccessPath accessPath) {
      key("offset");
      {
        ...
        if (accessPath.valid) {
          ...
          // key("cumulative");
          printComment("access path not available to compute cumulative offsets");
        }
      }
    }
    ...
    void key(char const* key) {
      static std::map<std::string, std::string> colors = {
          {"name", "\033[1;34m"},           // Blue
          {"kind", "\033[1;32m"},           // Green
          {"size", "\033[1;36m"},           // Cyan
          {"offset", "\033[1;35m"},         // Magenta
          {"cumulative", "\033[1;33m"},     // Yellow
          {"element count", "\033[1;31m"},  // Red
      };



      if (!afterArrayElement) { newLine(); }
      afterArrayElement = false;

      printf("%s: ", key);
      return;

      auto colorIt = colors.find(key);
      if (colorIt != colors.end()) {
        printf("%s%s\033[0m: ", colorIt->second.c_str(), key);
      } else {
        printf("%s: ", key);
      }
    }
...
#undef CASE
        ...
        default: printf("%d # unexpected enumerant", int(kind)); break;
      }
    }
...
#undef CASE
        ...
        default: printf("%d # unexpected enumerant", int(baseShape)); break;
      }
...
#undef CASE
        ...
        default: printf("%d # unexpected enumerant", int(access)); break;
      }
    }
...
#undef CASE
        ...
        default: printf("%d # unknown enumerant", int(layoutUnit)); break;
      }
    }
...
#undef CASE
        ...
        default: printf("%d # unexpected enumerant", int(stage)); break;
      };
    }
...
#undef CASE
        ...
        default: printf("%d # unhandled enumerant", int(targetFormat));
      }
    }
...
#undef CASE
        ...
        default: printf("%d # unhandled enumerant", int(scalarType));
      }
    }
...
#undef CASE
        ...
        default: printf("%d # unhandled enumerant", int(mode));
      }
    }
  };
}  // namespace ren
