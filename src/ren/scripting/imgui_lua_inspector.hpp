...
#define neko_bool_str(V) (V ? "true" : "false")
  ...
  inline bool incomplete_chunk_error(const char* err, std::size_t len) {
  ...
  struct luainspector_hints {
    ...
    static bool collect_hints_recurse(lua_State* L, std::vector<std::string>& possible,
                                      const std::string& last, bool usehidden, unsigned left);
    static void prepare_hints(lua_State* L, std::string str, std::string& last);
    static bool collect_hints(lua_State* L, std::vector<std::string>& possible,
  ...
  struct inspect_table_config {
    ...
    bool is_non_function = true;
  };

  enum luainspector_logtype {
    LUACON_LOG_TYPE_WARNING = 1,
    LUACON_LOG_TYPE_ERROR = 2,
    LUACON_LOG_TYPE_NOTE = 4,
    LUACON_LOG_TYPE_SUCCESS = 0,
    LUACON_LOG_TYPE_MESSAGE = 3
  };
  ...
  class luainspector {
   ...
   public:
    ...
    bool command_line_input(const char* label, std::string* str, ImGuiInputTextFlags flags = 0,
