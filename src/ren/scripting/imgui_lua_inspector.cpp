...
namespace neko {
  ...
  std::string luainspector_hints::clean_table_list(const std::string& str) {
    ...
    for (std::size_t i = 0u; i < str.size(); ++i) {
      ...
      if (c == '.' && got_white) {
        for (std::size_t j = 0u; j < (i - whitespace_start); ++j)
          ret.erase(--ret.end());
      }
  ...
  void luainspector_hints::prepare_hints(lua_State* L, std::string str, std::string& last) {
    ...
    for (std::size_t i = 0u; i < tables.size(); ++i) {
      ...
      if (lua_type(L, -1) != LUA_TTABLE && !luaL_getmetafield(L, -1, "__index") &&
  ...
  bool luainspector_hints::collect_hints_recurse(lua_State* L, std::vector<std::string>& possible,
                                                 const std::string& last, bool usehidden,
                                                 unsigned left) {
  ...
  bool luainspector_hints::collect_hints(lua_State* L, std::vector<std::string>& possible,
                                         const std::string& last, bool usehidden) {
    if (lua_type(L, -1) != LUA_TTABLE && !luainspector_hints::try_replace_with_metaindex(L))
      return false;
...
void neko::luainspector::print_luastack(int first, int last, luainspector_logtype logtype) {
  ...
  for (int i = first; i <= last; ++i) {
    ...
    switch (lua_type(L, i)) {
      ...
      default: ss << luaL_typename(L, i) << ": " << lua_topointer(L, i); break;
    }
    ss << ' ';
  }
...
void neko::luainspector::setL(lua_State* L) {
  ...
  // Backup the old print, since we override it with our own.
...
int neko::luainspector::command_line_callback_st(ImGuiInputTextCallbackData* data) noexcept {
  command_line_input_callback_UserData* user_data =
      (command_line_input_callback_UserData*)data->UserData;
  return reinterpret_cast<neko::luainspector*>(user_data->luainspector_ptr)
      ->command_line_input_callback(data);
}
...
int neko::luainspector::command_line_input_callback(ImGuiInputTextCallbackData* data) {
  ...
  auto paste_buffer = [data](auto begin, auto end, auto buffer_shift) {
    ...
    data->BufTextLen =
        std::min(static_cast<int>(std::distance(begin, end) + buffer_shift), data->BufSize - 1);
  ...
  command_beg =
      neko::find_terminating_word(cmd.data(), cmd.data() + cmd.size(),
...
bool neko::luainspector::command_line_input(const char* label, std::string* str,
  ...
  return ImGui::InputText(label, (char*)str->c_str(), str->capacity() + 1, flags,
                          command_line_callback_st, &cb_user_data);
}
...
void neko::luainspector::show_autocomplete() noexcept {
  ...
  this->m_input_focused = m_input_text_id == ImGui::GetActiveID();
  ...
  if ((m_input_text_id == ImGui::GetActiveID() || m_should_take_focus) &&
    ...
    if (ImGui::Begin("##terminal:auto_complete", nullptr, overlay_flags)) {
      ...
      if (max_displayable_sv < static_cast<long>(autocomplete_text.size())) {
        ...
        while (size >= 4 &&
               total_text_length + ImGui::CalcTextSize(buf.data(), buf.data() + size).x >=
      ...
      if (ImGui::IsKeyDown(ImGuiKey_Enter)) { cmd.clear(); }
    }
    ImGui::End();
    ImGui::PopStyleVar();
  }
...
void neko::luainspector::display(bool* textbox_react) noexcept {
  ...
  if (ImGui::BeginChild("##console_log", size)) {
    ...
    for (auto& a : messageLog) {
      ImVec4 colour;
      switch (a.second) {
        case LUACON_LOG_TYPE_WARNING: colour = {1.0f, 1.0f, 0.0f, 1.0f}; break;
        case LUACON_LOG_TYPE_ERROR: colour = {1.0f, 0.0f, 0.0f, 1.0f}; break;
        case LUACON_LOG_TYPE_NOTE: colour = {0.13f, 0.44f, 0.61f, 1.0f}; break;
        case LUACON_LOG_TYPE_SUCCESS: colour = {0.0f, 1.0f, 0.0f, 1.0f}; break;
        case LUACON_LOG_TYPE_MESSAGE: colour = {1.0f, 1.0f, 1.0f, 1.0f}; break;
      }
  ...
  if ((this->command_line_input(
           "##Input", &cmd,
           ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory) ||
       ImGui::IsItemActive()) &&
      textbox_react != nullptr) {
    *textbox_react = true;
  }
  ImGui::PopItemWidth();
    ...
    if (L) {
      ...
      if (evalok && LUA_OK == lua_pcall(L, 0, LUA_MULTRET, 0)) {
        if (oldtop != lua_gettop(L))
          print_luastack(oldtop + 1, lua_gettop(L), LUACON_LOG_TYPE_MESSAGE);
...
void neko::luainspector::print_line(const std::string& msg, luainspector_logtype type) noexcept {
...
void neko::luainspector::inspect_table(lua_State* L, inspect_table_config& cfg) {
  ...
  // check if we can inspect (must be key/value table)
  ...
  while (lua_next(L, -2) != 0) {
    ...
    bool edited = false;
    auto name = lua_tostring(L, -2);  // neko_lua_to<const char*>(L, -2);
    if (name == NULL || (cfg.search_str != 0 && !strstr(name, cfg.search_str))) {
      lua_pop(L, 1);
      continue;
    }
    ...
    if (cfg.is_non_function && type == LUA_TFUNCTION) { goto skip; }
    ...
    if (type == LUA_TSTRING) {
      ...
      const char* str_mem = lua_tostring(L, -1);
      size_t buffer_size = 256;
      for (; buffer_size < strlen(str_mem);)
        buffer_size += 128;
      std::string v(str_mem, buffer_size);
      ...
      ImGui::TreeNodeEx(name, tree_node_flags | ImGuiTreeNodeFlags_Leaf |
      ...
      ImGui::TextDisabled("%s", lua_typename(L, lua_type(L, -1)));
      ImGui::TableNextColumn();
      ImGui::TextColored(rgba_to_imvec(220, 160, 40, 255), "%s",
                         neko_bool_str(lua_toboolean(L, -1)));
    } else {
      ImGui::TreeNodeEx(name, tree_node_flags | ImGuiTreeNodeFlags_Leaf |
                                  ImGuiTreeNodeFlags_Bullet | ImGuiTreeNodeFlags_NoTreePushOnOpen);
      ImGui::TableNextColumn();
...
int neko::luainspector::draw(void) {
  neko::luainspector* model = this;  // (neko::luainspector*)lua_touserdata(L, 1);


  // toggle open on pressing ~ in SDL
  if (ImGui::IsKeyPressed(ImGuiKey_GraveAccent) && !m_input_focused) {
    open = !open;
    if (open) m_should_take_focus = true;
  }
  ...
  if (ImGui::Begin("LUA Console")) {
    if (ImGui::BeginTabBar("lua_inspector", ImGuiTabBarFlags_None)) {
      ...
      if (ImGui::BeginTabItem("Info")) {
        ...
        // ImGui::PlotLines("Frame Times", arr.data(), arr.size(), 0, NULL, 0, 4000,
        // ImVec2(0, 80.0f));
