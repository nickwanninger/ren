...
package.preload["fennel.repl"] = package.preload["fennel.repl"] or function(...)
    ...
    local function add_partials(input, tbl, prefix)
      ...
      for k, is_mangled in utils.allpairs(_775_()) do
        ...
        do
          ...
          if ((type(k) == "string") and (input == k:sub(0, #input)) and not seen[k] and
            ((":" ~= prefix:sub(-1)) or ("function" == type(tbl[lookup_k])))) then
            seen[k] = true
            val_19_ = (prefix .. k)
          else
            val_19_ = nil
  ...
  local function resolve(identifier, _825_0, scope)
    ...
    return _828_(pcall(compiler["compile-string"], tostring(identifier), {
      scope = scope
    }))
  end
  commands.find = function(env, read, on_values, on_error, scope)
    local function _836_(_241)
      ...
      if (((_G.type(_837_0) == "table") and (nil ~= _837_0.linedefined) and (nil ~= _837_0.source) and
        (_837_0.what == "Lua")) and _842_()) then
        local line = _837_0.linedefined
        local source = _837_0.source
        local fnlsrc = nil
        do
  ...
  local function repl(_3foptions)
    ...
    local callbacks = {
      ["view-opts"] = (opts["view-opts"] or {
        depth = 4
      }),
      env = env,
      onError = (opts.onError or default_on_error),
      onValues = (opts.onValues or default_on_values),
      pp = (opts.pp or view),
      readChunk = (opts.readChunk or default_read_chunk)
    }
    ...
    if save_locals_3f then
      ...
      env.___replLocals___ = setmetatable({}, {
        __newindex = newindex
      })
    end
...
package.preload["fennel.specials"] = package.preload["fennel.specials"] or function(...)
  ...
  local function compile_do(ast, scope, parent, _3fstart)
    ...
    for i = start, len do
      compiler.compile1(ast[i], sub_scope, parent, {
        nval = 0
      })
    end
    return nil
  ...
  local function set_fn_metadata(f_metadata, parent, fn_name)
    if utils.root.options.useMetadata then
      ...
      if (type(utils.root.options.useMetadata) == "string") then
        return compiler.emit(parent, ("%s:setall(%s, %s)"):format(utils.root.options.useMetadata, fn_name,
          table.concat(meta_fields, ", ")))
      else
        local meta_str = ("require(\"%s\").metadata"):format(fennel_module_name())
        return compiler.emit(parent, ("pcall(function() %s:setall(%s, %s) end)"):format(meta_str, fn_name,
          table.concat(meta_fields, ", ")))
      end
    end
  end
  ...
  local function compile_named_fn(ast, f_scope, f_chunk, parent, index, fn_name, local_3f, arg_name_list, f_metadata)
    ...
    for i = (index + 1), #ast do
      compiler.compile1(ast[i], f_scope, f_chunk, {
  ...
  local function get_function_metadata(ast, arg_list, index)
    ...
    return maybe_metadata(ast, utils["kv-table?"], _574_, maybe_metadata(ast, utils["string?"], _576_, {
      ["fnl/arglist"] = arg_list
    }, index))
  end
  SPECIALS.fn = function(ast, scope, parent)
    ...
    compiler.assert((not multi or not multi["multi-sym-method-call"]),
      ("unexpected multi symbol " .. tostring(fn_name)), fn_sym)
    ...
    local function destructure_arg(arg)
      ...
      compiler.destructure(arg, raw, ast, f_scope, f_chunk, {
        declaration = true,
        nomulti = true,
        symtype = "arg"
      })
    ...
    local function destructure_amp(i)
      compiler.assert((i == (#arg_list - 1)), "expected rest argument before last parameter", arg_list[(i + 1)],
        arg_list)
      f_scope.vararg = true
      compiler.destructure(arg_list[#arg_list], {utils.varg()}, ast, f_scope, f_chunk, {
        declaration = true,
        nomulti = true,
        symtype = "arg"
      })
  ...
  doc_special("fn", {"?name", "args", "?docstring", "..."},
  ...
  local function dot(ast, scope, parent)
    ...
    local _590_ = compiler.compile1(lhs_node, scope, parent, {
      nval = 1
    })
    local lhs = _590_[1]
    if (len == 2) then
      ...
      for i = 3, len do
        ...
        if (utils["string?"](index) and utils["valid-lua-identifier?"](index)) then
          ...
          local _591_ = compiler.compile1(index, scope, parent, {
            nval = 1
          })
  ...
  SPECIALS.let = function(_598_0, scope, parent, opts)
    ...
    local ast = _599_
    compiler.assert((utils["table?"](bindings) and not kv_3f(bindings)), "expected binding sequence",
      (bindings or ast[1]))
    ...
    for i = 1, #bindings, 2 do
      compiler.destructure(bindings[i], bindings[(i + 1)], ast, sub_scope, sub_chunk, {
        declaration = true,
        nomulti = true,
        symtype = "let"
      })
  ...
  doc_special("let", {{"name1", "val1", "...", "nameN", "valN"}, "..."},
    "Introduces a new scope in which a given set of local bindings are used.", true)
  ...
  SPECIALS.tset = function(ast, scope, parent)
    ...
    compiler.assert(((type(ast[2]) ~= "boolean") and (type(ast[2]) ~= "number")), "cannot set field of literal value",
      ast)
    local root = str1(compiler.compile1(ast[2], scope, parent, {
      nval = 1
    }))
    local root0 = nil
    if root:match("^[.{\"]") then
    ...
    do
      ...
      local i_18_ = #tbl_17_
      for i = 3, (#ast - 1) do
        local val_19_ = str1(compiler.compile1(ast[i], scope, parent, {
          nval = 1
        }))
        if (nil ~= val_19_) then
          i_18_ = (i_18_ + 1)
    ...
    local value = str1(compiler.compile1(ast[#ast], scope, parent, {
      nval = 1
    }))
    local fmtstr = nil
  ...
  doc_special("tset", {"tbl", "key1", "...", "keyN", "val"},
  ...
  local function if_2a(ast, scope, parent, opts)
    ...
    if (#ast == 2) then
      ...
      for i = 2, (#ast - 1), 2 do
        local condchunk = {}
        local _608_ = compiler.compile1(ast[i], do_scope, condchunk, {
          nval = 1
        })
        local cond = _608_[1]
  ...
  SPECIALS["if"] = if_2a
  doc_special("if", {"cond1", "body1", "...", "condN", "bodyN"},
  ...
  SPECIALS.each = function(ast, scope, parent)
    ...
    for raw, args in utils.stablepairs(destructures) do
      compiler.destructure(args, raw, ast, sub_scope, chunk, {
        declaration = true,
        nomulti = true,
        symtype = "each"
      })
  ...
  doc_special("each", {{"vals...", "iterator"}, "..."},
    "Runs the body once for each set of values provided by the given iterator.\nMost commonly used with ipairs for sequential tables or pairs for undefined\norder, but can be used with any iterator with any number of values.",
    true)
  local function while_2a(ast, scope, parent)
    local len1 = #parent
    local condition = compiler.compile1(ast[2], scope, parent, {
      nval = 1
    })[1]
    local len2 = #parent
    local sub_chunk = {}
  ...
  local function for_2a(ast, scope, parent)
    ...
    local range_args = {}
    local chunk = {}
    compiler.assert(utils["sym?"](binding_sym),
      ("unable to bind %s %s"):format(type(binding_sym), tostring(binding_sym)), ast[2])
    compiler.assert((3 <= #ast), "expected body expression", ast[1])
    ...
    for i = 1, math.min(#ranges, 3) do
      range_args[i] = str1(compiler.compile1(ranges[i], scope, parent, {
        nval = 1
      }))
    end
    compiler.emit(parent, ("for %s = %s do"):format(compiler["declare-local"](binding_sym, sub_scope, ast),
      table.concat(range_args, ", ")), ast)
  ...
  SPECIALS["for"] = for_2a
  doc_special("for", {{"index", "start", "stop", "?step"}, "..."},
  ...
  local function native_method_call(ast, _scope, _parent, target, args)
    ...
    if ((target.type == "literal") or (target.type == "varg") or
      ((target.type == "expression") and not (target[1]):match("[%)%]]$") and not (target[1]):match("%.[%a_][%w_]*$"))) then
      call_string = "(%s):%s(%s)"
    else
      call_string = "%s:%s(%s)"
    end
    return
      utils.expr(string.format(call_string, tostring(target), method_string, table.concat(args, ", ")), "statement")
  end
  local function nonnative_method_call(ast, scope, parent, target, args)
    local method_string = str1(compiler.compile1(ast[3], scope, parent, {
      nval = 1
    }))
    local args0 = {tostring(target), unpack(args)}
    return utils.expr(string.format("%s[%s](%s)", tostring(target), method_string, table.concat(args0, ", ")),
      "statement")
  end
  local function binding_method_call(ast, scope, parent, target, args)
    ...
    return
      utils.expr(string.format("(%s)[%s](%s)", target_local, method_string, table.concat(args0, ", ")), "statement")
  end
  local function method_call(ast, scope, parent)
    compiler.assert((2 < #ast), "expected at least 2 arguments", ast)
    local _627_ = compiler.compile1(ast[2], scope, parent, {
      nval = 1
    })
    ...
    for i = 4, #ast do
      ...
      subexprs = compiler.compile1(ast[i], scope, parent, {
        nval = _628_
      })
      local tbl_17_ = args
      local i_18_ = #tbl_17_
  ...
  SPECIALS.hashfn = function(ast, scope, parent)
    ...
    local function walker(idx, node, _3fparent_node)
      if utils["sym?"](node, "$...") then
        ...
        return ((utils["list?"](node) and (not _3fparent_node or not utils["sym?"](node[1], "hashfn"))) or
                 utils["table?"](node))
      end
    end
    utils["walk-tree"](ast, walker)
    compiler.compile1(ast[2], f_scope, f_chunk, {
      tail = true
    })
  ...
  local function short_circuit_safe_3f(x, scope)
    ...
    elseif utils["list?"](x) then
      if utils["sym?"](x[1]) then
        local _644_0 = str1(x)
        if ((_644_0 == "fn") or (_644_0 == "hashfn") or (_644_0 == "let") or (_644_0 == "local") or (_644_0 == "var") or
          (_644_0 == "set") or (_644_0 == "tset") or (_644_0 == "if") or (_644_0 == "each") or (_644_0 == "for") or
          (_644_0 == "while") or (_644_0 == "do") or (_644_0 == "lua") or (_644_0 == "global")) then
          return false
        elseif (((_644_0 == "<") or (_644_0 == ">") or (_644_0 == "<=") or (_644_0 == ">=") or (_644_0 == "=") or
          (_644_0 == "not=") or (_644_0 == "~=")) and (comparator_special_type(x) == "binding")) then
          return false
        else
  ...
  local function emit_short_circuit_if(ast, scope, parent, name, subast, accumulator, expr_string, setter)
    ...
    do
      local chunk = {}
      compiler.compile1(subast, scope, chunk, {
        nval = 1,
        target = accumulator
      })
  ...
  local function define_arithmetic_special(name, _3fzero_arity, _3funary_prefix, _3flua_name)
    ...
    SPECIALS[name] = _662_
    return
      doc_special(name, {"a", "b", "..."}, "Arithmetic operator; works the same as Lua but accepts more arguments.")
  end
  ...
  local function native_comparator(op, _674_0, scope, parent)
    ...
    local _676_ = compiler.compile1(lhs_ast, scope, parent, {
      nval = 1
    })
    local lhs = _676_[1]
    local _677_ = compiler.compile1(rhs_ast, scope, parent, {
      nval = 1
    })
    local rhs = _677_[1]
  ...
  local function binding_comparator(op, chain_op, ast, scope, parent)
    ...
    local chain = string.format(" %s ", (chain_op or "and"))
    for i = 2, #ast do
      local compiled = str1(compiler.compile1(ast[i], scope, parent, {
        nval = 1
      }))
    ...
    compiler.emit(parent, string.format("local %s = %s", table.concat(binding_left, ", "),
      table.concat(binding_right, ", "), ast))
    local _681_
    do
      local tbl_17_ = {}
      local i_18_ = #tbl_17_
  ...
  local function define_comparator_special(name, _3flua_op, _3fchain_op)
    ...
    return
      doc_special(name, {"a", "b", "..."}, "Comparison operator; works the same as Lua but accepts more arguments.")
  end
  ...
  local function define_unary_special(op, _3frealop)
    local function opfn(ast, scope, parent)
      compiler.assert((#ast == 2), "expected one argument", ast)
      local tail = compiler.compile1(ast[2], scope, parent, {
        nval = 1
      })
  ...
  local function make_compiler_env(_3fast, _3fscope, _3fparent, _3fopts)
    ...
    env = {
      ...
      ["fennel-module-name"] = fennel_module_name,
      ["get-scope"] = _693_,
      ["in-scope?"] = _694_,
      ["list?"] = utils["list?"],
  ...
  local function sandbox_fennel_module(modname)
    if ((modname == "fennel.macros") or (package and package.loaded and ("table" == type(package.loaded[modname])) and
      (package.loaded[modname].metadata == compiler.metadata))) then
  ...
  SPECIALS["require-macros"] = function(ast, scope, parent, _3freal_ast)
    ...
    if not macro_loaded[modname] then
      ...
      macro_loaded[modname] = compiler.assert(utils["table?"](loader(modname, filename)), "expected macros to be table",
        (_3freal_ast or ast))
    end
  ...
  doc_special("require-macros", {"macro-module-name"},
  ...
  SPECIALS.include = function(ast, scope, parent, opts)
    ...
    do
      ...
      if ((_746_0 == true) and (nil ~= _747_0)) then
        ...
        local _ = _746_0
        modexpr = compiler.compile1(ast[2], scope, parent, {
          nval = 1
        })[1]
      end
    end
    if ((modexpr.type ~= "literal") or ((modexpr[1]):byte() ~= 34)) then
      ...
      res = ((utils["member?"](mod, (utils.root.options.skipInclude or {})) and opts.fallback(modexpr, true)) or
              include_circular_fallback(mod, modexpr, opts.fallback, ast) or utils.root.scope.includes[mod] or _751_())
      utils.root.options["module-name"] = oldmod
      return res
    end
  end
  doc_special("include", {"module-name-literal"},
  ...
  doc_special("macros", {"{:macro-name-1 (fn [...] ...) ... :macro-name-N macro-body-N}"},
  ...
  SPECIALS["pick-values"] = function(ast, scope, parent)
    ...
    compiler.assert((("number" == type(n)) and (0 <= n) and (n == math.floor(n))),
      ("Expected n to be an integer >= 0, got " .. tostring(n)))
    if (1 == n) then
      local _755_ = compiler.compile1(vals, scope, parent, {
        nval = 1
      })
      local _756_ = _755_[1]
      local expr = _756_[1]
    ...
    elseif (0 == n) then
      for i = 3, #ast do
        compiler["keep-side-effects"](compiler.compile1(ast[i], scope, parent, {
          nval = 0
        }), parent, nil, ast[i])
      end
      return {}
  ...
  return {
    ...
    ["get-function-metadata"] = get_function_metadata,
    ["load-code"] = load_code,
    ["macro-loaded"] = macro_loaded,
...
package.preload["fennel.compiler"] = package.preload["fennel.compiler"] or function(...)
  ...
  local function make_scope(_3fparent)
    ...
    return {
      ["gensym-base"] = setmetatable({}, {
        __index = (parent and parent["gensym-base"])
      }),
      autogensyms = setmetatable({}, {
        __index = (parent and parent.autogensyms)
      }),
      depth = _300_,
      gensyms = setmetatable({}, {
        __index = (parent and parent.gensyms)
      }),
  ...
  local function check_binding_valid(symbol, scope, ast, _3fopts)
    ...
    assert_compile(not (scope.specials[(part1 or name)] or (not macro_3f and scope.macros[(part1 or name)])),
      ("local %s was overshadowed by a special form or macro"):format(name), ast)
    assert_compile((not macro_3f or not part1 or not scope.macros[part1]),
      "tried to set multisym macro on existing macro", ast)
    return assert_compile(not utils["quoted?"](symbol), string.format("macro tried to bind %s without gensym", name),
      symbol)
  end
  ...
  local function symbol_to_expression(symbol, scope, _3freference_3f)
    ...
    assert_compile((not scope.specials[parts[1]] or ("require" == parts[1])),
      "tried to reference a special form without calling it", symbol)
    assert_compile((not _3freference_3f or local_3f or ("_ENV" == parts[1]) or global_allowed_3f(parts[1])),
  ...
  local function peephole(chunk)
    if chunk.leaf then
      return chunk
    elseif ((3 <= #chunk) and (chunk[(#chunk - 2)].leaf == "do") and not chunk[(#chunk - 1)].leaf and
      ...
      do
        ...
        do
          ...
          for l in debug.traceback(msg, 2):gmatch("([^\n]+)") do
            if l:find("function 'fennel.compiler.macroexpand'$") then
              break
            end
            local val_19_ = l
            if (nil ~= val_19_) then
  ...
  local function compile_function_call(ast, scope, parent, opts, compile1, len)
    local _411_ = compile1(ast[1], scope, parent, {
      nval = 1
    })[1]
    ...
    for i = 2, len do
      ...
      subexprs = compile1(ast[i], scope, parent, {
        nval = _412_
      })
  ...
  local function compile_call(ast, scope, parent, opts, compile1)
    ...
    elseif (multi_sym_parts and multi_sym_parts["multi-sym-method-call"]) then
      ...
      local new_ast = utils.list(utils.sym(":", ast), utils.sym(table_with_method, ast), method_to_call,
        select(2, unpack(ast)))
  ...
  local function compile_sym(ast, scope, parent, opts)
    ...
    assert_compile(not (multi_sym_parts and multi_sym_parts["multi-sym-method-call"]),
  ...
  local function compile_table(ast, scope, parent, opts, compile1)
    local function escape_key(k)
      if ((type(k) == "string") and utils["valid-lua-identifier?"](k)) then
        ...
        local _426_ = compile1(k, scope, parent, {
          nval = 1
        })
    ...
    do
      ...
      for i, elem in ipairs(ast) do
        ...
        do
          ...
          keys[i] = true
          val_19_ = exprs1(compile1(elem, scope, parent, {
            nval = nval
          }))
        end
        if (nil ~= val_19_) then
    ...
    do
      ...
      for k in utils.stablepairs(ast) do
        local val_19_ = nil
        if not keys[k] then
          local _429_ = compile1(ast[k], scope, parent, {
            nval = 1
          })
          local v = _429_[1]
    ...
    return
      handle_compile_opts({utils.expr(("{" .. table.concat(buffer, ", ") .. "}"), "expression")}, parent, opts, ast)
  end
  ...
  local function destructure(to, from, ast, scope, parent, opts)
    ...
    local function getname(symbol, ast0)
      ...
      if declaration then
        ...
        if ((#parts == 1) and not forceset) then
          assert_compile(not (forceglobal and meta), string.format("global %s conflicts with local", tostring(symbol)),
            symbol)
          assert_compile(not (meta and not meta.var), ("expected var " .. raw), symbol)
        end
        assert_compile((meta or not opts0.noundef or (scope.hashfn and ("$" == first)) or global_allowed_3f(first)),
          ("expected local " .. first), symbol)
        if forceglobal then
          assert_compile(not scope.symmeta[scope.unmanglings[raw]], ("global " .. raw .. " conflicts with local"),
            symbol)
    ...
    local function compile_top_target(targets)
      ...
      local ret = compile1(from, scope, parent, {
        target = target
      })
      if declaration then
    ...
    local function destructure_close(left, up1)
      ...
      return compile1(from, scope, parent, {
        target = target
      })
    end
    local function dynamic_set_target(_450_0)
      ...
      do
        ...
        for _0, k in ipairs(keys) do
          local val_19_ = tostring(compile1(k, scope, parent, {
            nval = 1
          })[1])
          if (nil ~= val_19_) then
            i_18_ = (i_18_ + 1)
    ...
    local function destructure_table(left, rightexprs, top_3f, destructure1, up1)
      assert_compile((("table" == type(rightexprs)) and not utils["sym?"](rightexprs, "nil")),
        "could not destructure literal", left)
      if optimize_table_destructure_3f(left, rightexprs) then
        return destructure_values(utils.list(unpack(left)), utils.list(utils.sym("values"), unpack(rightexprs)), up1,
          destructure1)
      else
        local right = nil
        do
  ...
  local function traceback(_3fmsg, _3fstart)
    ...
    if ((_504_0 == "nil") or (_504_0 == "string")) then
      local msg = (_3fmsg or "")
      if ((msg:find("^%g+:%d+:%d+: Compile error:.*") or msg:find("^%g+:%d+:%d+: Parse error:.*")) and
  ...
  local function do_quote(form, scope, parent, runtime_3f)
    ...
    elseif utils["sym?"](form) then
      ...
      if (symstr:find("#$") or symstr:find("#[:.]")) then
        return string.format("_G.sym('%s', {filename=%s, line=%s})", autogensym(symstr, scope), filename,
          (form.line or "nil"))
      else
        return
          string.format("_G.sym('%s', {quoted=true, filename=%s, line=%s})", symstr, filename, (form.line or "nil"))
      end
    ...
    elseif utils["list?"](form) then
      ...
      return string.format(("setmetatable({filename=%s, line=%s, bytestart=%s, %s}" .. ", getmetatable(_G.list()))"),
        filename, (form.line or "nil"), (form.bytestart or "nil"), mixed_concat(mapped, ", "))
    elseif utils["sequence?"](form) then
      ...
      if runtime_3f then
        ...
        return string.format("setmetatable({%s}, {filename=%s, line=%s, sequence=%s})", mapped_str, filename,
          (source.line or "nil"), "(getmetatable(_G.sequence()))['sequence']")
      end
    elseif (type(form) == "table") then
      ...
      return string.format("setmetatable({%s}, {filename=%s, line=%s})", mixed_concat(quote_all(form), ", "), filename,
        _527_())
  ...
  return {
    ["apply-deferred-scope-changes"] = apply_deferred_scope_changes,
    ...
    ["do-quote"] = do_quote,
    ["global-allowed?"] = global_allowed_3f,
...
package.preload["fennel.friend"] = package.preload["fennel.friend"] or function(...)
  ...
  pal("cannot call literal value",
    {"checking for typos", "checking for a missing function name", "making sure to use prefix operators, not infix"})
  pal("could not compile value of type ", {"debugging the macro you're calling to return a list or table"})
  pal("could not read number (.*)", {"removing the non-digit character",
                                     "beginning the identifier with a non-digit if it is not meant to be a number"})
  pal("expected a function.* to call",
    {"removing the empty parentheses", "using square brackets if you want an empty table"})
  ...
  pal("expected even number of pattern/body pairs",
    {"checking that every pattern has a body to go with it", "adding _ before the final body"})
  ...
  pal("expected var (.*)",
    {"declaring %s using var instead of let/local", "introducing a new local instead of changing the value of %s"})
  ...
  pal("invalid character: (.)",
    {"deleting or replacing %s", "avoiding reserved characters like \", \\, ', ~, ;, @, `, and comma"})
  ...
  pal("may only be used at compile time", {"moving this to inside a macro if you need to manipulate symbols/lists",
                                           "using square brackets instead of parens to construct a table"})
  pal("method must be last component",
    {"using a period instead of a colon for field access", "removing segments after the colon",
     "making the method call, then looking up the field on the result"})
  pal("mismatched closing delimiter (.), expected (.)",
    {"replacing %s with %s", "deleting %s", "adding matching opening delimiter earlier"})
  pal("missing subject", {"adding an item to operate on"})
  pal("multisym method calls may only be in call position",
    {"using a period instead of a colon to reference a table's fields", "putting parens around this"})
  pal("tried to reference a macro without calling it", {"renaming the macro so as not to conflict with locals"})
  pal("tried to reference a special form without calling it", {"making sure to use prefix operators, not infix",
  ...
  pal("unknown identifier: (.*)",
    {"looking to see if there's a typo", "using the _G table instead, eg. _G.%s if you really want a global",
     "moving this code to somewhere that %s is in scope", "binding %s as a local in the scope of this code"})
  pal("unused local (.*)", {"renaming the local to _%s if it is meant to be unused", "fixing a typo so %s is used",
                            "disabling the linter which checks for unused locals"})
  pal("use of global (.*) is aliased by a local",
    {"renaming local %s", "refer to the global using _G.%s instead of directly"})
  ...
  local function highlight_line(codeline, col, _3fendcol, opts)
    if ((opts and (false == opts["error-pinpoint"])) or (os and os.getenv and os.getenv("NO_COLOR"))) then
      ...
      local open = _204_[1]
      local close = _204_[2]
      return (sub(codeline, 1, col) .. open .. sub(codeline, (col + 1), (endcol + 1)) .. close ..
  ...
  local function assert_compile(condition, msg, ast, source, opts)
    if not condition then
      ...
      error(friendly_msg(("%s:%s:%s: Compile error: %s"):format((filename or "unknown"), (line or "?"), (col or "?"),
        msg), utils["ast-source"](ast), source, opts), 0)
    end
    return condition
  end
  local function parse_error(msg, filename, line, col, endcol, source, opts)
    return error(friendly_msg(("%s:%s:%s: Parse error: %s"):format(filename, line, col, msg), {
      col = col,
      endcol = endcol,
      endline = line,
      filename = filename,
...
package.preload["fennel.parser"] = package.preload["fennel.parser"] or function(...)
  ...
  local function sym_char_3f(b)
    ...
    return
      ((32 < b0) and not delims[b0] and (b0 ~= 127) and (b0 ~= 34) and (b0 ~= 39) and (b0 ~= 126) and (b0 ~= 59) and
        (b0 ~= 44) and (b0 ~= 64) and (b0 ~= 96))
  end
  local prefixes = {
    [35] = "hashfn",
    [39] = "quote",
  ...
  local function parser_fn(getbyte, filename, _223_0)
    ...
    local function parse_error(msg, _3fcol_adjust)
      ...
      if (nil ==
        utils["hook-opts"]("parse-error", options, msg, filename, (line or "?"), col0, source, utils.root.reset)) then
        utils.root.reset()
        if unfriendly then
    ...
    local function parse_stream()
      ...
      local function encode_utf8(codepoint_str)
        ...
        if (nil ~= _261_0) then
          ...
          elseif ((2048 <= codepoint) and (codepoint <= 65535)) then
            return string.char((224 + bitrange(codepoint, 12, 16)), (128 + bitrange(codepoint, 6, 12)),
              (128 + bitrange(codepoint, 0, 6)))
          elseif ((65536 <= codepoint) and (codepoint <= 2097151)) then
            return string.char((240 + bitrange(codepoint, 18, 21)), (128 + bitrange(codepoint, 12, 18)),
              (128 + bitrange(codepoint, 6, 12)), (128 + bitrange(codepoint, 0, 6)))
          elseif ((131072 <= codepoint) and (codepoint <= 67108863)) then
            return string.char((248 + bitrange(codepoint, 24, 26)), (128 + bitrange(codepoint, 18, 24)),
              (128 + bitrange(codepoint, 12, 18)), (128 + bitrange(codepoint, 6, 12)), (128 + bitrange(codepoint, 0, 6)))
          elseif ((4194304 <= codepoint) and (codepoint <= 2147483647)) then
            return string.char((252 + bitrange(codepoint, 30, 31)), (128 + bitrange(codepoint, 24, 30)),
              (128 + bitrange(codepoint, 18, 24)), (128 + bitrange(codepoint, 12, 18)),
              (128 + bitrange(codepoint, 6, 12)), (128 + bitrange(codepoint, 0, 6)))
          else
      ...
      local function parse_number(rawstr, source0)
        ...
        elseif rawstr:match("^%d") then
          dispatch((tonumber(trimmed) or parse_error(("could not read number \"" .. rawstr .. "\""), (-#rawstr))),
            source0, rawstr)
          return true
        else
...
package.preload["fennel.view"] = package.preload["fennel.view"] or function(...)
  ...
  local function table_kv_pairs(t, options)
    if (("number" ~= type(options["max-sparse-gap"])) or
      (options["max-sparse-gap"] ~= math.floor(options["max-sparse-gap"]))) then
  ...
  local function visible_cycle_3f(t, options)
    return (getopt(options, "detect-cycles?") and detect_cycle(t, {}) and save_table(t, options.seen) and
             (1 < (options.appearances[t] or 0)))
  end
  ...
  local function concat_table_lines(elements, options, multiline_3f, indent, table_type, prefix, last_comment_3f)
    ...
    if (not getopt(options, "one-line?") and
      (multiline_3f or (options["line-length"] < (indent + length_2a(oneline))) or last_comment_3f)) then
      local function _41_()
        ...
        for _, _45_0 in ipairs(kv) do
          ...
          do
            ...
            multiline_3f = (multiline_3f or k0:find("\n") or v0:find("\n") or
                             (options0["line-length"] < length_2a((k0 .. " " .. v0))))
            val_19_ = {k0, v0}
          end
          if (nil ~= val_19_) then
            i_18_ = (i_18_ + 1)
    ...
    local function validate_utf8(str0, index)
      ...
      do
        ...
        for _, init0 in ipairs(inits) do
          ...
          ret = (byte and (function(_84_, _85_, _86_)
            return (_84_ <= _85_) and (_85_ <= _86_)
          end)(init0["min-byte"], byte, init0["max-byte"]) and init0)
        end
        init = ret
      end
      local code = nil
      ...
      code = (init and _87_())
      if (code and (function(_89_, _90_, _91_)
        return (_89_ <= _90_) and (_90_ <= _91_)
      end)(init["min-code"], code, init["max-code"]) and not ((55296 <= code) and (code <= 57343))) then
        return init.len
      end
    end
    local index = 1
  ...
  local function pp_string(str, options, indent)
    ...
    local esc_newline_3f = ((len < 2) or
                             (getopt(options, "escape-newlines?") and (len < (options["line-length"] - indent))))
    local byte_escape = (getopt(options, "byte-escape") or default_byte_escape)
    local escs = nil
    local _95_
...
package.preload["fennel.utils"] = package.preload["fennel.utils"] or function(...)
  ...
  local function luajit_vm_3f()
    return ((nil ~= _G.jit) and (type(_G.jit) == "table") and (nil ~= _G.jit.on) and (nil ~= _G.jit.off) and
             (type(_G.jit.version_num) == "number"))
  end
  ...
  local function fengari_vm_3f()
    return ((nil ~= _G.fengari) and (type(_G.fengari) == "table") and (nil ~= _G.fengari.VERSION) and
  ...
  local function sequence(...)
    local function _152_(seq, view0, inspector, indent)
      local opts = nil
      do
        inspector["empty-as-sequence?"] = {
          after = inspector["empty-as-sequence?"],
          once = true
        }
        inspector["metamethod?"] = {
          after = inspector["metamethod?"],
          once = false
        }
        opts = inspector
      end
  ...
  local function table_3f(x)
    return
      ((type(x) == "table") and not varg_3f(x) and (getmetatable(x) ~= list_mt) and (getmetatable(x) ~= symbol_mt) and
        not comment_3f(x) and x)
  end
  ...
  local function multi_sym_3f(str)
    ...
    elseif (type(str) ~= "string") then
      ...
      return ((str:match("%.") or str:match(":")) and not str:match("%.%.") and (str:byte() ~= string.byte(".")) and
               (str:byte() ~= string.byte(":")) and (str:byte(-1) ~= string.byte(".")) and
               (str:byte(-1) ~= string.byte(":")) and _160_())
    end
  end
  ...
  local function check_plugin_version(_182_0)
    ...
    if (not member_3f(version:gsub("-dev", ""), (versions or {})) and
      not (string_3f(versions) and version:find(versions)) and not warned[plugin]) then
      warned[plugin] = true
  ...
  return {
    ...
    ["list?"] = list_3f,
    ["lua-keyword?"] = lua_keyword_3f,
    ["macro-path"] = table.concat({"./?.fnlm", "./?/init.fnlm", "./?.fnl", "./?/init-macros.fnl", "./?/init.fnl",
...
local function syntax()
  local body_3f = {"when", "with-open", "collect", "icollect", "fcollect", "lambda", "\206\187", "macro", "match",
                   "match-try", "case", "case-try", "accumulate", "faccumulate", "doto"}
