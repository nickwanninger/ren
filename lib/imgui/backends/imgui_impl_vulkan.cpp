...
//  '(ImGui_ImplXXXX_RenderState*)GetPlatformIO().Renderer_RenderState'. [x] Renderer:
//  Multi-viewport / platform windows. With issues (flickering when creating a new viewport).
...
//  2025-06-11: Vulkan: Added support for ImGuiBackendFlags_RendererHasTextures, for dynamic font
//  atlas. Removed ImGui_ImplVulkan_CreateFontsTexture() and ImGui_ImplVulkan_DestroyFontsTexture().
//  2025-05-07: Vulkan: Fixed validation errors during window detach in multi-viewport mode. (#8600,
//  #8176) 2025-05-07: Vulkan: Load dynamic rendering functions using vkGetDeviceProcAddr() + try
//  both non-KHR and KHR versions. (#8600, #8326, #8365) 2025-04-07: Vulkan: Deep-copy
...
struct ImGui_ImplVulkan_ViewportData {
  ...
  bool WindowOwned;
  bool SwapChainNeedRebuild;  // Flag when viewport swapchain resized in the middle of processing a
                              // frame
...
void ImGui_ImplVulkan_UpdateTexture(ImTextureData* tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    backend_tex->DescriptorSet = ImGui_ImplVulkan_AddTexture(
        bd->TexSampler, backend_tex->ImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info) {
  ...
  // Our render function expect RendererUserData to be storing the window render buffer we need (for
  // the main viewport we won't use ->Window)
...
void ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance, VkPhysicalDevice physical_device,
  ...
  ImGui_ImplVulkanH_CreateWindowSwapChain(physical_device, device, wd, allocator, width, height,
                                          min_image_count);
  // ImGui_ImplVulkan_CreatePipeline(device, allocator, VK_NULL_HANDLE, wd->RenderPass,
  // VK_SAMPLE_COUNT_1_BIT, &wd->Pipeline, g_VulkanInitInfo.Subpass);
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport) {
  ...
  #endif
  const VkFormat defaultFormats[] = {VK_FORMAT_B8G8R8A8_UNORM, VK_FORMAT_R8G8B8A8_UNORM,
  ...
  ImGui_ImplVulkanH_CreateOrResizeWindow(v->Instance, v->PhysicalDevice, v->Device, wd,
                                         v->QueueFamily, v->Allocator, (int)viewport->Size.x,
                                         (int)viewport->Size.y, v->MinImageCount);
  vd->WindowOwned = true;

  // Create pipeline (shared by all secondary viewports)
  if (bd->PipelineForViewports == VK_NULL_HANDLE)
    ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, VK_NULL_HANDLE, wd->RenderPass,
                                    VK_SAMPLE_COUNT_1_BIT, &bd->PipelineForViewports, 0);
}

static void ImGui_ImplVulkan_DestroyWindow(ImGuiViewport* viewport) {
  // The main viewport (owned by the application) will always have RendererUserData == 0 since we
...
static void ImGui_ImplVulkan_SetWindowSize(ImGuiViewport* viewport, ImVec2 size) {
  ...
  if (vd ==
      nullptr)  // This is nullptr for the main viewport (which is left to the user/app to handle)
    return;
  ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
  vd->Window.ClearEnable = (viewport->Flags & ImGuiViewportFlags_NoRendererClear) ? false : true;
  ImGui_ImplVulkanH_CreateOrResizeWindow(v->Instance, v->PhysicalDevice, v->Device, &vd->Window,
                                         v->QueueFamily, v->Allocator, (int)size.x, (int)size.y,
                                         v->MinImageCount);
}
  ...
  {
    {
      ...
      if (err == VK_ERROR_OUT_OF_DATE_KHR) {
        vd->SwapChainNeedRebuild = true;  // Since we are not going to swap this frame anyway, it's
                                          // ok that recreation happens on next frame.
        return;
      }
    ...
    if (v->UseDynamicRendering) {
      ...
      vkCmdPipelineBarrier(
          fd->CommandBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_NONE,
  ...
  #endif
    {
      ...
      info.pClearValues =
          (viewport->Flags & ImGuiViewportFlags_NoRendererClear) ? nullptr : &wd->ClearValue;
